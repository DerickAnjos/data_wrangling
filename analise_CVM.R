# Dataset Comissão de Valores Mobiliários (CVM)
...
summarise(base_cvm_aj, observacoes = n(),
          média = mean(VARIACAO, na.rm = T))
...
base_cvm_aj <- base_cvm_aj %>% filter((infinitos == F & nan == F))
...
base_cvm_atual <- base_cvm_aj %>% 
  mutate(ano = substr(DT_FIM_EXERC, 1, 4),
         VALORES_ATUAL = if_else(ano == '2019',
                                 ...
                                 VALORES*1)) %>% 
  group_by(CD_CVM, CD_CONTA) %>% 
  mutate(VARIACAO_ATUAL = ((VALORES_ATUAL - lag(VALORES_ATUAL, n=1L))/
                             lag(VALORES_ATUAL, n=1L))) %>% 
  ungroup() %>% droplevels(.) %>% 
